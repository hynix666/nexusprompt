...
| **The manifest reader is a heuristic, not a parser.** Seven sweeps, each finding new ambiguous Markdown shapes. **12** known limits remain, enumerated in `spec/manifest-shapes.json` and re-derived by `check:truth`: sub-headings, emphasised and numbered headings, tilde dividers, blockquoted and checkbox declarations, setext and HTML headings, HTML tables, a commented key counted as a use, and a comment opener inside a fence — all erring toward FAIL, a visible refusal an author clears. **Exactly one errs the other way** and has across three sweeps: a manifest inside a fenced warning block returns PASS, and a silent PASS is the direction that ships defects | Either a real block-structure parse, or the accept-set is declared closed and everything outside it FAILs by design |
