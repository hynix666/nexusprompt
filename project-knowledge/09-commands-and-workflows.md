...
| `npm run check:hash` | The artifact hash: 78 runtime files, LF-normalised so a CRLF and an LF checkout of one commit agree. Fails when a runtime source changed without the hash being regenerated |
| `npm run check:truth` | Re-derives the 12 truth-boundary entries from the tree. Fails when a boundary moves — the first provider answer, a divergence retired, a known limit fixed |
