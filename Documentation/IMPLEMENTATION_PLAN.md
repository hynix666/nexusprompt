...
**The real trap was not the one R7 named, and it is now closed.** A *nine-stage* copy sat unfrozen in the repository root — `SystemPromptBuilderPipeline.tsx`, SHA-256 `c519e72b…`, no `Cost Estimate`, no `Tone Check`. Its mtime was **newer** than the frozen one, so "check mtimes" pointed the wrong way and only content distinguished them. It and `SystemPromptBuilderPipeline-Unified.tsx` were deleted on 30 August 2026; the frozen eleven-stage component at `sources/pipeline/` is the only copy in the tree, and `check:stages` derives from it. Both remain in git history for anyone tracing the lineage.
