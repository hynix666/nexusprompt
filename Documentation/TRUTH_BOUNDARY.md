...
## Gate firings are counted and labelled; no precision figure is reported yet

`gate-precision-is-unmeasured` · probe `precisionSurface`

**Establishes.** That the question now has data behind it. 124 firings over 1,513 compiled prompts from eight models carry a TRUE/FALSE label with a reason, checked by check:precision, which fails when a firing has no label, when a label describes no firing, or when a corpus file changed after it was judged. The labels are concentrated, not scattered: SECRET_LEAK_SCAN fired 84 times and every credential-shaped match is a literal copied from the brief, while CLAIM_DISCIPLINE fired 19 times and not one of those sentences asserts a guarantee.

**Does not establish.** That any gate has a measured precision. No interval has been computed (Phase 9, Task 4), and a ratio quoted without one would be the point estimate this phase exists to avoid, on gates whose counts run from 1 firing to 84. Every label is the judgement of an uncalibrated model, made by Claude alone under the owner's D1 decision and reviewed by nobody: ADR-0008 refuses exactly this kind of judge elsewhere in the system, and `adjudicated_by` records it on all 124 records. The figure is also corpus-relative in two ways: the briefs are generated, half of the pilot slice plants a hazard on purpose, and three of the eight models come from endpoints that could not finish a run. None of it says how a gate behaves on prompts a person wrote.
...
**Crossed when.** The corpus and the labels were both crossed on 12 September 2026, and this entry rewritten rather than renumbered each time. adjudicated_firings moving means a gate started firing somewhere new, a model was added, or a label was revisited — each of which changes what the counts describe. It becomes a measured claim only when Task 4 computes an interval per gate, and this entry must then pin n and the bounds rather than a rounded ratio.
