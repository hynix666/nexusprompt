...
## Local and hosted models have answered; nothing this repository REPORTS came from one
...
**Does not establish.** Any claim about how a language model behaves, and in particular nothing about the figures this repository reports. EVERY evaluation number here still comes from the pinned stub — the anchor's 4,906 cases, each smoke suite, every gate recall figure — and they remain evidence about this system's accounting rather than about a model's output. What crossing the boundary bought is narrower than it sounds: 6 models compiling ONE brief at LOW stakes, with no expected output, no scoring, and no comparison. Every gate returned PASS on all of them, which says the compiled prompts satisfy static checks, not that any model is good or that two models differ. Those runs are also not reproducible — models are stochastic, temperature is not pinned, and the bundles are gitignored, so nothing in this tree re-derives them and CI has never seen one. Hosted models HAVE now answered, which this entry denied until 12 September 2026: `build:precision-corpus --hosted` drove `provider-hosted-server`'s compatible dialect against third-party OpenAI-compatible endpoints, and their compiled prompts are committed under `eval/precision-corpus/`. That is narrower than it sounds, and narrower than the `--live` path it is often confused with. The Anthropic `--live` evaluation path has STILL never executed, so the four refusals above remain untested by use rather than by design. Those hosted runs were free-tier or credit-funded, so no spend has been authorised or made. Two of the endpoints are reseller proxies (`kiraai.vn`, `api.unorouter.com`, `api.orcarouter.ai`) whose served model this repository cannot verify: a corpus records what an endpoint served under a name, never that the named model produced it. And a corpus is stored text, not a reported figure — no number in this repository comes from any of it until the precision entry says otherwise. No prompt-engineering advice in the documentation set has been measured against a model.
...
**Establishes.** All sixteen ported gates are compared verdict-for-verdict against the frozen Python linter they were ported from, and the linter is SHA-256 pinned in the source freeze so it cannot be edited into agreement. Where the port deliberately differs, the difference is declared with a reason and an ADR rather than reconciled or hidden — ten such divergences, from five ADRs. Two fix a source defect; ADR-0017 does not, it extends; and ADR-0020 and ADR-0021 each fix one the oracle could never have found, because the source shares it — the distinction matters to what this entry claims. Those two were found by measuring the port against a corpus instead, which is the only route to a defect an oracle agrees with. Its four divergences EXTEND SECRET_LEAK_SCAN past the shapes the source scans for, so on those four the oracle is not being told the source is wrong — it is being told the port looks for more. Agreement with the source is therefore no longer available as evidence about them in either direction.
...
**Does not establish.** That the system in the 61 documentation files exists. Those were written target-state and in the present tense before any code existed, so a sentence saying the platform 'implements' something is a specification, not a report. The count moved from 44 to 53 on 2 September 2026, and the direction is the finding: all nine additions are further target-state planning documents for adapters, shells and type hardening that remain unbuilt, so the ratio this entry bounds got worse while no code moved. The count moved again from 53 to 54 on 3 September 2026 with ADR-0016 (mutation-derived judge calibration), which documents a deliberate divergence from the production guidance's expectation of human-labeled calibration data. It moved from 54 to 57 on 6 September 2026 — ADR-0017, ADR-0018 and ADR-0019 — and those three are a different kind of addition: each records a decision about code that was built and shipped, so unlike the nine planning documents they do not widen the distance between what is described and what exists. It moved from 57 to 59 on 9 September 2026, with a repository audit and a minimum-change remediation strategy; the audit measures what is built rather than specifying more of it, while the remediation strategy is forward-looking and does widen that distance. It moved 59 to 60 on 12 September 2026 with ADR-0020, and 60 to 61 the same day with ADR-0021; both record a decision about code that shipped in the same commit, so like the three ADRs before them they do not widen the distance this entry bounds. The opening sentence of this entry sat at 59 while the pin read 60, because the ADR-0020 commit moved the number the checker reads and not the number a reader reads — which is the same failure mode in miniature that this whole document exists to prevent, and is why the narrative is kept in step here rather than only the count. Naming the difference is not a softening, and this entry does not grant the newer files an exemption: they are counted here exactly like the rest, because the moment a category of documentation stops counting is the moment this number stops bounding anything. Documentation growth here is not progress and must not be read as any — an audit of the built system is measurement, not movement, and a remediation strategy is a plan, not a remedy. Two adapters of the five the documentation specifies are absent — `provider-hosted-server` and `storage-db` — and note that the built set and the target set are NOT the same set: `content-local`, `provider-ollama`, and `provider-hosted-judge` were built here and are in neither documented five, so six built against a target of five does not mean the target is met. `provider-hosted-judge` is not from this plan's phases at all — it is the first real transport for the separate brief-fidelity-judge sub-project — which is exactly the kind of addition this sentence exists to keep visible rather than let flatter the ratio. Reading those two numbers as a fraction is the mistake this sentence exists to prevent. Two Shells of four — `pipeline-ui` and `toolkit-ui` — have no code at all. `shells/api` counts as built as of 29 August 2026 (ADR-0012): it compiles, is typechecked with everything else, and its routes and socket seam are tested. It is also the newest and least exercised thing here, and it is the only part of the repository with runtime dependencies. What the two built Shells lack is presentation, not capability — both drive the full pipeline through the Application protocol — which is why the gap is stated as scope rather than as a shortfall.

**Pinned:**

```json
{
  "documentation_markdown_files": 61,
  "gates_built": 16,
  "stages_built": 11,
  "adapters_built": [
    "content-local",
...
**Establishes.** Three things reproduce here, and they are reported separately because they are not equally strong. (1) SAME INPUT, SAME VERDICTS: the gates are pure, denied every effectful builtin by a static check that reads every file under core/src, and 2,848 oracle verdicts agree with a SHA-256-pinned second implementation. (2) SAME SEED, SAME SUITE: the 4,906-case anchor regenerates from seed 1 and check:anchor fails if the committed file is not what the generator produces. (3) SAME SOURCE, SAME HASH: 99 artifact files digest to one hash, and content is normalised to LF first because core.autocrlf is true here — so the hash a Windows checkout computes is the hash a Linux checkout computes, which is the only version of the claim worth making. The count moved 84→86 across two commits that each added one runtime file — `core/src/eval/brief-fidelity.ts`, then `adapters/provider-hosted-judge/src/index.ts` — and sat wrong at 85 in between: the first addition should have moved it to 85 and did not, caught by neither that commit's implementer nor its reviewer, so the second inherited an already-stale base and landed on 85 instead of the true 86. A third commit, adding `core/src/eval/judge-calibration.ts`, repeated the identical mechanism rather than a new one: `npm run build:hash` was run before `git add`, so `git ls-files` still reported the pre-commit tree and the stale 86 was committed alongside a file that made the true count 87 — caught by review rather than by that commit's own author, and corrected in the commit after it. A fourth commit, adding `application/src/judge-bundle.ts`, staged the new file before hashing and moved the count cleanly to the true 88. A fifth commit, adding `core/src/eval/compare-graded.ts` (sub-project 4's paired-bootstrap comparator), staged the new file before hashing and moved the count cleanly to the true 89. A sixth commit, adding `application/src/judge-pilot.ts`, did the same and moved it to the true 90. The count drifted the same way three separate times before that, always because the hash was computed before the new file was staged, which is itself evidence that re-deriving the count is doing real work here rather than merely confirming what was already believed. Phase 5 additions (provider-hosted-server and provider-hosted-judge adapter sources) moved the count from 90 to 92 across two commits, both staged cleanly before hashing. Phase 6 (pipeline-ui and toolkit-ui shells) added six src files but ran `build-hash.mjs --write` before `git add`, so `git ls-files` reported the pre-commit tree and the count was committed as 92 instead of the true 98 — the fourth recurrence of the same mechanism, caught by CI rather than by the author. The correct count is 98, restored in the commit immediately following the CI failure. A FIFTH recurrence landed on 6 September 2026 with `shells/api/src/security.ts`, by the identical mechanism: `build-hash.mjs --write` was run while the new file was still untracked, `git ls-files` reported the pre-commit tree, and a local `verify` passed at 98 on a tree CI derived 99 from. Caught by CI, as the fourth was. Five recurrences of one mechanism is not five lapses of attention — the count is derived from the INDEX while the file sits in the working tree, so a full local verify is green precisely up until the moment the file is staged, and nothing in `build:hash` says so. That is a defect in the tool rather than in its users, and the honest place to record it is here, where the number it keeps getting wrong is pinned. Closed on 6 September 2026: `build-hash.mjs` now lists untracked-but-not-ignored files with `git ls-files --others --exclude-standard`, filters them through the same `isArtifactPath` predicate the hash uses, and refuses to write or check while any remain — so the sixth recurrence cannot reach a commit, let alone CI. There is no override flag, because the two honest answers are to stage the file or to ignore it.
...
## Gate precision is measured on one generated corpus, by an uncalibrated judge

`gate-precision-measured-on-one-corpus` · probe `precisionSurface`

**Establishes.** How often five of the sixteen gates are right when they fire, on 1,513 compiled prompts from eight models. Each of the 92 firings carries a TRUE/FALSE label with a reason, and `check:precision` reports an exact (Clopper-Pearson) interval per gate rather than a ratio: SECRET_LEAK_SCAN 69/84 (72.3%-89.6%), DUPLICATE_INSTRUCTION 0/3 (0.0%-70.8%), GUARDRAIL_GAP 2/2 (15.8%-100%), RUNTIME_KEY_UNDECLARED 2/2 (15.8%-100%), PLACEHOLDER_AUDIT 1/1 (2.5%-100%). One false-positive class remains measured rather than anecdotal: DUPLICATE_INSTRUCTION flags the worked example the compile template demands. Two were fixed because of this measurement, and each is a declared divergence from the frozen linter rather than a quiet edit: CLAIM_DISCIPLINE fired 19 times, every one a sentence DENYING a guarantee, and gate 1.2.0 now reads polarity (ADR-0020), so it fires nowhere here; GUARDRAIL_GAP fired 15 times, 13 of them on a clause that was PRESENT but spelled with a Unicode dash, and gate 1.1.0 now matches a clause's own dash against Unicode's Dash_Punctuation category (ADR-0021), which silenced exactly those 13 and left both true firings intact. The check fails if a firing loses its label, a label loses its firing, or a corpus file's text changes after it was judged.

**Does not establish.** That any gate has a precision, full stop. Eleven of the sixteen produced no firing here, and they are silent for two different reasons that must not be read as one. THREE ARE NOT A PRECISION QUESTION AT ALL: TOKEN_BUDGET, QUTM_CEILING and CONTEXT_LIMIT compare a token estimate against a number the CALLER declares — a budget, a cost ceiling against a baseline, a provider's context limit. `tokens > budget` is true or false, so there is no wrong-but-fired to count; only a policy set well or badly, which no corpus can adjudicate. Calling them unmeasured implied a measurement was owed. What IS worth knowing about them was measured: armed at medium stakes with no naiveTokens declared, QUTM_CEILING FAILs 572 of the 1,513 prompts against a baseline the caller never chose (the source's built-in 400), and gate 1.1.1 now names that baseline in the message rather than reporting a bare ratio. THREE MORE WERE NEVER ARMED: RECURSION_MACHINERY_PRESENT, RAG_SHIELD_GAP and ADVERSARIAL_RESILIENCE return PASS immediately unless a target or a corpus is supplied, and nothing in the product supplies one, so their silence says nothing whatever about what the models wrote. FIVE WERE ARMED on all 1,513 prompts and stayed quiet, which is a measurement of a kind: CLAIM_DISCIPLINE (after ADR-0020 its precision is unmeasured rather than 0.0%-17.6%, and measuring it again needs prompts that assert a guarantee), DELIMITER_ENTROPY, ORPHAN_CLAIMS, SOURCE_LEDGER_MISSING and TOKEN_SPAM, and a gate absent from the table has no measurement rather than a good one. Where it did fire the counts are mostly small: three gates rest on 1 or 2 firings, whose intervals reach 100% because nothing else is excludable at that size, and only SECRET_LEAK_SCAN has an interval narrower than 30 points. GUARDRAIL_GAP's 2/2 is the clearest case of why a point estimate must never be read on its own here: it is what was left after ADR-0021 removed a false-positive class, not evidence that the gate is right when it fires, and its interval still runs from 15.8% to 100%. Removing a wrong answer is not the same as acquiring a right one, and this entry got narrower, not stronger. Every label is the judgement of an uncalibrated model made by Claude alone and reviewed by nobody, which ADR-0008 refuses for scoring elsewhere in this system. The figure belongs to THIS corpus: briefs generated from a template, half the pilot slice planting a credential or a stand-in on purpose, and three of the eight models drawn from endpoints that could not finish a run. Precision on prompts a person wrote is unmeasured. Recall and precision here come from different corpora and do not compose into an F-score or any other single number.
...
```json
{
  "precision_by_gate": [
    ...
    "RUNTIME_KEY_UNDECLARED 2/2",
    "SECRET_LEAK_SCAN 69/84"
  ],
  "precision_corpus_exists": true,
  "adjudicated_firings": 92
}
```

**Crossed when.** A gate moving between armed-and-silent and not-armed-at-all is a change in what the corpus measures, not in what the gates do, and `check:precision` derives that distinction from each gate's own `.not_armed` code rather than a list. Any count above moves: a gate starts or stops firing, a model is added to the corpus, or a label is revisited. Each changes what the intervals describe, and the bounds are re-derived from these counts rather than stored, so the entry cannot quote a percentage that has drifted from its own data. A human review that overturns a label would also move it, and would strengthen the warrant this entry currently disclaims.

**Evidence:** `eval/precision-corpus` · `eval/precision-adjudications.json` · `scripts/check-precision.ts` · `core/src/eval/precision.ts`
