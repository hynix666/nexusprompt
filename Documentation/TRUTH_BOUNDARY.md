...
**Crossed when.** The corpus half was crossed on 12 September 2026 and this entry rewritten rather than renumbered. What remains is `adjudicated_firings` leaving 0, in `eval/precision-adjudications.json` (Phase 9, Task 3). The entry is then rewritten as a measured claim that pins, per gate, the number of firings and the exact interval bounds — never a rounded point estimate — and says the figure belongs to that corpus, not to the gate.
