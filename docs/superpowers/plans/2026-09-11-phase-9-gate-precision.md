...
- **D2, amended the same day by the owner.** `qwen3.8:27b` took 571 s for one brief (the other three took 11–56 s), which is about 30 h for 200 briefs, and at a 10-minute timeout its longest answers would be excluded, biasing the corpus. It was swapped for `gemma4:e4b` (44.5 s, measured while the GPU was shared). The owner then added one **hosted** model, `nvidia/nemotron-3-super-120b-a12b` on NVIDIA's OpenAI-compatible endpoint. That is the first corpus whose briefs leave this machine, which D2 had ruled out; the briefs are synthetic and hold nothing sensitive. It runs through `provider-hosted-server`'s `compatible` dialect with `--hosted`. The key is read only from the operator's environment. The builder pins the model, because Core names `claude-opus-5` on every request, and spaces calls 5.1 s apart under the adapter's 12-per-minute limit. Once it has answered, the truth-boundary entry `no-model-has-answered` ("No hosted model has answered") must be rewritten in the same commit as that corpus.
