...
- `Documentation/` — 61 Markdown files describing the *target* architecture of the platform, plus four review documents assessing it. This is still the bulk of the repository, and it still describes a system far larger than what exists — nine of the files landed on 2 September 2026 as further planning documents, so the gap widened rather than closed.
...
- Four source archives, still zipped, loose on disk. The two loose nine-stage copies of `SystemPromptBuilderPipeline.tsx` that used to sit beside them were **deleted on 30 August 2026** — they were stale duplicates of the frozen eleven-stage component at `sources/pipeline/SystemPromptBuilderPipeline.tsx`, and their mtimes were NEWER than it, so "check mtimes" pointed the wrong way. Only content distinguished them. They remain in git history.
...
1. **The port is complete: all sixteen gates are ported and compared.** ADR-0007's divergence allowlist exists as of 18 August 2026 — `scripts/divergence-allowlist.json`, enforced by `scripts/differential.ts`. A port that deliberately fixes a source defect declares the difference with a reason and an ADR instead of choosing between reproducing the bug and deleting the oracle.
   ...
   It holds **ten entries as of 12 September 2026**. Three are ADR-0010 (the runtime manifest was a span to end-of-file, so a *use* declared itself and the gate returned PASS on undeclared keys — the same defect `extractSourceLedgerIds` already carries a fix for, plus a third entry closing the CommonMark heading-whitespace gap the fifth adversarial sweep found); one is ADR-0011 (`QUTM_CEILING` divided a compiled prompt by the one-line brief it came from, so no compliant artifact could clear it); four are ADR-0017 (`SECRET_LEAK_SCAN` extended past the source's own pattern set to catch a JWT, a PEM private key, a URL carrying embedded credentials, and a Stripe key — the source is not wrong about the shapes it carries, it simply does not carry these). One is ADR-0020 (`CLAIM_DISCIPLINE` reads polarity: the source flags a sentence that *denies* a guarantee exactly like one asserting it, and the precision corpus showed all 19 of its firings were denials). That entry is about polarity and nothing else. One is ADR-0021 (`GUARDRAIL_GAP` matches a clause’s own dash against Unicode’s `Pd` category: the source finds `anti-override` only when the dash is U+002D, and 13 of the gate’s 15 firings on the precision corpus were a clause that was PRESENT but spelled with U+2011). That entry is about the dash and nothing else — `anti override` and `anti_override` still read as absent in both implementations, because widening this gate can only cost it sensitivity. Do **not** add the `CLAIM_DISCIPLINE` / `guarantee-free` case ADR-0007 names as a candidate: a hyphen is a word boundary, nothing in that phrase is a denial, the source shares the false positive, so there is still no divergence and the entry would fail the stale rule immediately.
   ...
   ~~Still open and larger than it looks: **there is no git remote.**~~ **Closed 23 August 2026** — `origin` is `git@github.com:hynix666/nexusprompt.git`, and `.github/workflows/verify.yml` ran green on its first push. Phase 7 is unblocked.

   **The repository is PUBLIC.** Five documents called it private in six places — this file included — until 4 September 2026; `gh api repos/hynix666/nexusprompt` reports `visibility: "public"`. Nothing in the tree records *when* it changed, so this states what is true rather than inventing a date, and the dated closure notes above have had the word struck rather than reversed. Load-bearing in a way a wrong count is not: the frozen sources, every commit message and the whole audit trail are world-readable. `master` carries branch protection for that reason — a green `verify` required, enforced on admins, no direct pushes.
...
3. ~~**`storage-db` revision persistence is new work, not a port.**~~ **Closed 5 September 2026** — the inherited Drizzle schema (MySQL) had `users` and `promptAssets` and no revisions table, so it was built fresh: `adapters/storage-db` implements `RevisionStore` via `node:sqlite`, not a port of the Drizzle schema (PR [#115](https://github.com/hynix666/nexusprompt/pull/115)).
4. ~~**Neither scaffolding generator exists.**~~ **Closed 5 September 2026** — `scripts/new-gate.ts` and `scripts/new-technique.py` both exist, wired as `npm run scaffold:gate` / `npm run scaffold:technique` (PR [#116](https://github.com/hynix666/nexusprompt/pull/116)). Contributors can be told to use them.
...
| `npm run check:schema-self-reference` / `ack:` | Version numbers a schema writes into its own prose, pinned to the `$id` they were read under. It reads none of the English — `judge-verdict`'s "Added in 1.2.0" is permanently true and `run-manifest`'s "this schema is at 2.0.0" rotted, and no matcher can tell them apart, so it fails on the **bump** instead and prints the sentences. Re-read them, then `npm run ack:schema-self-reference`. |
| `npm run docs:truth` / `check:truth` | The truth boundary: 12 entries stating what this repository establishes and what it does **not**, each pinned to numbers re-derived from the tree. Reading `Documentation/TRUTH_BOUNDARY.md` first is the cheapest way to avoid quoting a figure more broadly than it holds. |
